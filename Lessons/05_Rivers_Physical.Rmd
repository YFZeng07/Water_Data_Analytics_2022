...
library(EcoHydRology) #Load it first because some share functions
library(tidyverse)
...
ggplot(EnoDischarge, aes(x = Baseflow_lfstat, y = bt)) +
  ...
  geom_abline(slope = 1, intercept = 0, color = "darkred") #1:1 line for reference
...
```{r}
#Import Coolidge discharge data
Coolidge <- readNWISdv(siteNumbers = "07137500",
                     parameterCd = "00060", # discharge (ft3/s)
                     startDate = "1960-10-01", 
                     endDate = "2005-09-30")

names(Coolidge)[4:5] <- c("Discharge", "Approval.Code")
