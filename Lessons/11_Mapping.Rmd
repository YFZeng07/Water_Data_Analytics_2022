...
secchi.summary <- LAGOScombined %>%
  filter(sampledate > "2011-12-31") %>% #Last 5 years
  group_by(lagoslakeid) %>%
  summarise(secchi.mean = mean(secchi),
            area = mean(lake_area_ha),
            depth = mean(maxdepth),
            lat = mean(nhd_lat),  #mean of lat and long to keep them in the df
            long = mean(nhd_long)) %>%
  drop_na()
...
secchi.summary.min <- LAGOScombined %>%
  filter(sampledate > "2011-12-31") %>% #Last 5 years
  group_by(lagoslakeid) %>%
  summarise(secchi.mean = min(secchi),
            area = mean(lake_area_ha),
            depth = mean(maxdepth),
            lat = mean(nhd_lat),  #mean of lat and long to keep them in the df
            long = mean(nhd_long)) %>%
...
#secchi.min.spatial <- st_as_sf(secchi.summary.min, coords = c("long", "lat"), crs = 4326)
...
```{r}
Maine.secchi.spatial <- st_as_sf(Maine.secchi, coords = c("long", "lat"), crs = 4326)

Maine.state <- filter(states, ID %in% "maine")

P1 <- ggplot() +
  geom_sf(data = Maine.state, fill = "white") +
  geom_sf(data = Maine.secchi.spatial, aes(color = secchi.mean, size = area), 
          alpha = 0.5) +
  scale_color_viridis_c(trans = "log10", end = 0.8, direction = -1) +
  labs(color = "Average Secchi Depth (m)") +
  theme(legend.position = "top")
...
P2 <- ggplot() +
  geom_sf(data = Maine.state, fill = "white") +
  geom_sf(data = Maine.secchi.spatial, aes(color = secchi.mean, size = depth), 
          alpha = 0.5) + #various: in the aes(). Fixed: outside of aes()
  scale_color_viridis_c(trans = "log10", end = 0.8, direction = -1) +
  labs(color = "Average Secchi Depth (m)") +
  theme(legend.position = "top")
P2
