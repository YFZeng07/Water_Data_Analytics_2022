...
#read_csv under dply package is faster and more efficient for bigger dataset
...
dim(NTLdata)
NTLdata <- NTLdata %>% #telling what dataset is so that ↓
  drop_na(temperature_C) #no need to indicate the column by $
#Alternative in one line
#NTLdata <- drop_no(NTLdata, temperature_C) #but it is easier to add/change with the pipeline %>%
...
NTLdata_PeterPaul <- NTLdata %>%
  filter(lakename == "Paul Lake" | lakename == "Peter Lake") #|: or 
...
epilimnion (up) and hypolimnion (lower)

- dimictic lake: 2 states/mixes in a year
- monomictic: 1 mixing and only stratified in summer (bc not cold enough in winter)
- polymictic: mixes several times a year (mixing when strong wind event etc) (bc shallower <-> dimictic is deep and shelter)
- amictic: no mixing
...
ggplot(Pauldata_thermo, aes(x = daynum, y = thermocline, col = year4)) +
  geom_point() +
  scale_color_viridis_c()
